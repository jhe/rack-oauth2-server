  ...
  module OAuth2
    ...
    protected
      ...
      def respond_with_access_token(request, logger)
        ...
        begin
          ...
          when "assertion"
            ...
            logger.error request.POST.inspect.to_s
            logger.error 'request.query_string.to_s = '
            logger.error request.query_string.to_s
            logger.error 'request.body.inspect.to_s = '
            logger.error request.body.inspect.to_s
            logger.info "**********************************************************************************************"
      ...
      def get_client(request, options={})
        ...
        if request.env['HTTP_X_OAUTH_SECRET']
          client_id = request.env['HTTP_X_OAUTH_CLIENT']
          client_secret = request.env['HTTP_X_OAUTH_SECRET']
        elsif request.basic?
